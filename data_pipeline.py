...
from src.deployment.batch.data_flow import data_flow
from src.deployment.batch.prepare_daily_data import data_prep_flow
...
def data_processing_job(data_url, params, db_token, running_date: str, logger) -> None:
    ...
    inference_flag = data_prep_flow(db_token=db_token, date=running_date, logger=logger)
    if inference_flag:
        logger.info("Inference Can be Started Safely")
    else:
